...
public class InitApplicationListener implements ApplicationListener {
    ...
    public boolean onStart(ApplicationContext ctx) throws Exception {
        InputStream inputStream = Resources.getResourceAsStream("config/mybatis-config.xml");//getResourceAsStream를 쓰면 클래스패스의 루트에서 mybatis-config를 찾는다.
        SqlSessionFactoryBuilder sqlSessionFactoryBuilder = new SqlSessionFactoryBuilder();
        SqlSessionFactory sqlSessionFactory = sqlSessionFactoryBuilder.build(inputStream);// Factory는 의존객체를 다 알고 있음
        SqlSession sqlSession = sqlSessionFactory.openSession(false); //자동 커밋 취소

        UserDao userDao = new UserDaoImpl(sqlSession);// 위에서 sqlSesstion이 각각의 Dao에 공급한다.
