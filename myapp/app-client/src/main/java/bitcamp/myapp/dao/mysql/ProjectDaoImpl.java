...
public class ProjectDaoImpl implements ProjectDao {
  ...
  public boolean insert(Project project) throws Exception {
    try (Statement stmt = con.createStatement()) {
      ...
      // 자동 생성된 PK 컬럼 값 가져올 때 사용할 ResultSet 객체를 준비 한다.
      ResultSet keyRS = stmt.getGeneratedKeys();

      //ResultSet 객체를 통해 서버에서 PK 값을 가져온다.
      // => Project를 한 개 insert 했기 떄문에 생성된 PK 값도 한 개다.
      // => 따라서 가져올 PK 값도 한 개다.
      keyRS.next();

      // PK 컬럼이 한 개이기 때문에 0번째 컬럼 값을 꺼낸다.
      // PK 컬럼을 가져올 때는 컬럼 이름으로 꺼내는 것이 아니라 순번으로 꺼낸다.
      int projectNo = keyRS.getInt(1);

      //호출할 쪽에서 project PK 값을 사용할 수 있도록 project 객체의 담아 둔다.
