...
public class ProjectDaoImpl implements ProjectDao {
  ...
  public boolean update(Project project) throws Exception {
    int count = sqlSession.update("update myapp_projects set"
  ...
  public boolean delete(int no) throws Exception {
    int count = sqlSession.delete("delete from myapp_projects where project_id=?",
        no);
    return count > 0;
  }
  ...
  public boolean insertMembers(int projectNo, List<User> members) throws Exception {
    for (User user : members) {
      sqlSession.insert("insert into myapp_project_members(project_id, user_id) values (?, ?)",
          projectNo,
          user.getNo());
    }
    return true;
  ...
  public boolean deleteMembers(int projectNo) throws Exception {
    int count = sqlSession.delete("delete from myapp_project_members where project_id=?",
        projectNo);
    return count > 0;
  }
}
